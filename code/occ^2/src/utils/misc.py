...
import time
from src.utils.comm import get_local_rank
...
def setup_gpus(gpus: Union[str, int], num_nodes, logdir, filename) -> int:
    """A temporary fix for pytorch-lighting 1.3.x"""

    if num_nodes > 1:
        os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str(i) for i in range(gpus))
        os.environ["NODE_RANK"] = os.getenv("RLAUNCH_REPLICA")
        os.environ["WORLD_SIZE"] = str(gpus * num_nodes)
        # os.environ["LOCAL_RANK"] = str(get_local_rank())
        # os.environ["MASTER_PORT"] = "29500"

        os.makedirs(logdir, exist_ok=True)
        filepath = os.path.join(logdir, filename + ".txt")
        print("hostname path:", filepath)
        if os.getenv("RLAUNCH_REPLICA") == "0":
            set_master(filepath)
        os.environ["MASTER_ADDR"] = get_master(filepath)
