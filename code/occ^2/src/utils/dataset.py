...
def imread_gray(path, augment_fn=None, client=SCANNET_CLIENT, nori=False):
    ...
    if not nori:
        ...
        image = cv2.imdecode(np.frombuffer(io_buf.getbuffer(), np.uint8), cv_type)
    ...
    if augment_fn is not None:
        if not nori:
            ...
            io_buf = io.BytesIO(data)
            image = cv2.imdecode(
                np.frombuffer(io_buf.getbuffer(), np.uint8), cv2.IMREAD_COLOR
            )
...
def read_scannet_gray(path, resize=(640, 480), augment_fn=None, nori=False):
    """
    Args:
    ...
    image = imread_gray(path, augment_fn, nori=nori)
...
def read_scannet_depth(path, resize=(640, 480), nori=False):
    if nori:
        data = nf.get(path)
        io_buf = io.BytesIO(data)
        depth = cv2.imdecode(
            np.frombuffer(io_buf.getbuffer(), np.uint8), cv2.IMREAD_UNCHANGED
        )
    else:
