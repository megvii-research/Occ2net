...
class ResNetFPN_8_2(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layer3_outconv2 = conv1x1(block_dims[1], block_dims[2])
        self.layer3_outconv3 = conv1x1(block_dims[0], block_dims[2])
    ...
    def forward(self, x):
        ...
        # FPN
        x2_h = F.interpolate(x2, scale_factor=0.5, mode="bilinear", align_corners=True)
        x1_h = F.interpolate(x1, scale_factor=0.25, mode="bilinear", align_corners=True)
        x2_h = self.layer3_outconv2(x2_h)
        x1_h = self.layer3_outconv3(x1_h)
        x3_out = self.layer3_outconv(x3 + x2_h + x1_h)
