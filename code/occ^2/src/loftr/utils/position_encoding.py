...
class PE3(nn.Module):
    ...
    def __init__(self, d_model=18, max_shape=(64, 256, 320), temp_bug_fix=True):
        """
        Args:
            max_shape (tuple): for 1/8 featmap, the max length of 256 corresponds to 2048 pixels
            temp_bug_fix (bool): As noted in this [issue](https://github.com/zju3dv/LoFTR/issues/41),
        ...
        if temp_bug_fix:
            div_term = torch.exp(
                torch.arange(0, d_model // 3, 2).float()
                * (-math.log(10000.0) / (d_model // 3))
            )
        else:  # a buggy implementation (for backward compatability only)
            div_term = torch.exp(
                torch.arange(0, d_model // 3, 2).float()
        ...
        pe[4::6, :, :] = torch.cos(y_position * div_term)
        pe[5::6, :, :] = torch.cos(y_position * div_term)
