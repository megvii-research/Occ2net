...
def spvs_coarse(data, config):
    ...
    if config["LOFTR"]["MATCH_COARSE"]["PATCH_CENTER"]:
        grid_pt0_c += 0.5
        grid_pt1_i += 0.5
    ...
    valid_mask_0, occluded_mask_0, non_occluded_mask_0, w_pt0_i = warp_kpts(
        grid_pt0_i,
        data["depth0"],
        data["depth1"],
        data["T_0to1"],
    ...
    valid_mask_1, occluded_mask_1, non_occluded_mask_1, w_pt1_i = warp_kpts(
        grid_pt1_i,
        data["depth1"],
        data["depth0"],
        data["T_1to0"],
    ...
    # correct_0to1 = non_occluded_mask_0
    if config["LOFTR"]["SPVS_MASK"] == "loop_back":
        correct_0to1 = loop_back == torch.arange(h0 * w0, device=device)[None].repeat(
            N, 1
        )
    elif config["LOFTR"]["SPVS_MASK"] == "valid":
        correct_0to1 = valid_mask_0
    elif config["LOFTR"]["SPVS_MASK"] == "nonocc":
        correct_0to1 = non_occluded_mask_0
