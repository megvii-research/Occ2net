...
class CoarseMatching(nn.Module):
    ...
    def forward(
        ...
        rotate_feature = feat_c0_rotate is not None
        ...
        if self.match_type == "dual_softmax":
            if not rotate_feature:
                ...
                sim_matrix_r0 = (
                    torch.einsum("nlc,nsc->nls", feat_c0_rotate, feat_c1)
                    / self.temperature
                )
                sim_matrix_0r = (
                    torch.einsum("nlc,nsc->nls", feat_c0, feat_c1_rotate)
                    / self.temperature
                )
                # sim_matrix_rr = torch.einsum("nlc,nsc->nls", feat_c0_rotate, feat_c1_rotate) / self.temperature
                sims = torch.stack([sim_matrix_00, sim_matrix_r0, sim_matrix_0r], dim=0)
                sim_matrix = torch.sum(
                    (sims * F.gumbel_softmax(sims, dim=0, hard=False)), dim=0
                )
                conf_matrix = F.softmax(sim_matrix, 1) * F.softmax(sim_matrix, 2)
