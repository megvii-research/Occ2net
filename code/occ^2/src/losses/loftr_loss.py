...
class LoFTRLoss(nn.Module):
    ...
    def compute_depth_loss(self, depth0, gt0, depth1, gt1):
        loss = ((depth0 - gt0) ** 2 + (depth1 - gt1) ** 2).mean()
        return loss
    ...
    def forward(self, data):
        ...
        # 3. coarse depth loss
        loss_depth = self.compute_depth_loss(data["occ_f0"], data["occ_g0"], data["occ_f1"], data["occ_g1"])
        if loss_depth is not None:
            loss += loss_depth * 0.1
            loss_scalars.update({"loss_depth":loss_depth.clone().detach().cpu()})
        else:
            assert self.training is False
            loss_scalars.update({"loss_depth": torch.tensor(1.0)})  # 1 is the upper bound
