...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def _setup_dataset(
        ...
        if scene_list_path is None:
            npz_dir = os_path_join(split_npz_root, mode)
            npz_names = os_glob(os_path_join(npz_dir, "*.npz"))
        else:
            with open(scene_list_path, "r") as f:
                npz_names = [name.split()[0] for name in f.readlines()]

        if mode == "train":
            # npz_names = npz_names[::15]
    ...
    def _build_concat_dataset_parallel(
        ...
        if str(data_source).lower() == "megadepth":
            npz_names = [f"{n}.npz" for n in npz_names]
        if mode == "train" and data_source == "ScanNet":
            scene_exists = [
                p.split("/")[-1] for p in glob(os.path.join(data_root, "*"))
            ]
            npz_names = [name for name in npz_names if name[:-4] in scene_exists]
        with tqdm_joblib(
            tqdm(
                desc=f"[rank:{self.rank}] loading {mode} datasets",
                total=len(npz_names),
        ...
        print("############## datasets length: ", sum([len(d) for d in datasets]))
        return ConcatDataset(datasets)
